...
export function FAQItemsManager() {
  ...
  const API_BASE_URL = '/api/admin/content/faq_items'
  ...
  async function handleSave() {
    ...
    if (!response.ok) {
      ...
      alert(
        `Error ${isExisting ? 'updating' : 'creating'} FAQ item${error?.details ? `: ${error.details}` : ''}`
      )
      return
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Billing, Support, Technical, etc."
            />
          </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold">Existing FAQ Items</h3>
        
        {items.length === 0 ? (
          <Card className="p-6 text-center text-muted-foreground">
            No FAQ items found. Create one above.
          </Card>
        ) : (
          items.map((item) => (
